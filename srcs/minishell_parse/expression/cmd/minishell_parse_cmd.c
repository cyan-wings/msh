...
static t_ast	*minishell_parse_cmd_error(
		...
		t_ast **cmd_node)
{
	minishell_parse_astfree(exec_node);
	minishell_parse_astfree(args_node);
	minishell_parse_astfree(redirs_node);
	minishell_parse_astfree(cmd_node);
	return (0);
}
 ...
 * 		the outer function (i.e., minishell_parse_cmd).
 *
 ...
 */
static int	minishell_parse_cmd_helper(
		t_list **token_ptr,
		t_ast **redirs_node,
		t_ast **exec_node,
		t_ast **args_node
		)
{
	...
	*redirs_node = minishell_parse_astnew("redirections", 0);
	if (*token_ptr && ((t_token *)(*token_ptr)->content)->type == REDIR_OP)
		status = minishell_parse_cmd_redirections(token_ptr, redirs_node);
	if (!status || \
...
t_ast	*minishell_parse_cmd(t_list **token_ptr)
{
	...
	status = minishell_parse_cmd_helper(token_ptr, \
	...
	return (minishell_parse_cmd_error(&exec_node, \
